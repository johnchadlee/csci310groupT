...
@Test
	public void TestLoginValid() throws Exception {
    	...
    	when(request.getRequestDispatcher("login.html")).thenReturn(rd);
    	when(request.getRequestDispatcher("authentication.html")).thenReturn(rd);
	    when(request.getSession()).thenReturn(session);
	    
        when(request.getParameter("email")).thenReturn("bsexton");//mocking
        when(request.getParameter("password")).thenReturn("ben");//mocking
...
public void TestLoginInvalid() throws Exception {
    ...
    when(request.getParameter("email")).thenReturn("Benjamin");//mocking
    when(request.getParameter("password")).thenReturn("ben");//mocking
...
//    verify(request, atLeast(1)).getParameter("email"); // only if you want to verify username was called...
    writer.flush(); // it may not have been flushed yet...
   System.out.println(stringWriter.toString());
    assertTrue(stringWriter.toString().contains("Sorry, email or password error!"));
}
}
